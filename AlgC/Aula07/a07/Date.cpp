...
int DateCompare(const Date* a, const Date* b) {
  if(((int)a->year - (int)b->year) != 0){
    return (int)a->year - (int)b->year;
  }
  else if(((int)a->month - (int)b->month) != 0){
...
void DateDecr(Date* d) {
  ...
  if((int)d->day <2){
    if((int)d->month > 1){
      d->month = (d->month) -1;
      d->day =DateDaysInMonth((int)d->year,d->month);
    }
    else{
      d->month=12;
      d->year= (int)(d->year)-1;
      d->day=DateDaysInMonth((int)(d->year)-1,d->month);
    }
  }
  else{
    d->day=(int)(d->day)-1;
