//NMEC: 93086
//NOME: Lúcia Maria Bessa de Sousa
//
...
// A: Post-order é a melhor ordem para aqui, pois é a melhor maneira de destruir uma arvore 
// removemos os nós mais distantes antes de remover o nó root
...
int BSTreeContains(const BSTree* header, const void* item) {
  ...
  while (root != NULL) {
    ...
    if((header->compare(root->item,item))==0) return 1;
    else if((header->compare(root->item,item))< 0) root= root->right;
    else root = root->left;
  }
  return 0;
}
...
// R: A árvore tem n nós e está quilibrada, logo, podemos concluir que a função
// search, insertion e remove têm ordem de complexidade logaritmica, O(log(n))
// 
// Explique como poderia evitar a chamada a _getNumNodes nesta função.
// Q: O que seria preciso mudar na estrutura de dados?
// R: Poderia ser criada uma estrutura de dados, uma lista ou fila, onde 
// se armazenavam os nós e, dexariamos de chamar a função _getNumNodes e 
// passariamos a chamar a função size().
// Q: E que funções teriam de ser modificadas?
// R: BSTreeAdd, BSTreeRemove, BSTreeCreate e _treeDestroy
//
//
